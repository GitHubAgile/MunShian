...
public class AddStaffJAVA extends JFrame{
    ...
    public AddStaffJAVA(){
        ...
        int count=0;
     try{ while(rs2.next()){
            jlblCurrentID.setText("S"+(Integer.valueOf(rs2.getString(1).replaceAll("S",""))+1));
            count++;
        }
        if(count==0){
            jlblCurrentID.setText("S1");
        }
    ...
    public void ClearText(){
     ...
     try{ while(rs3.next()){
            jlblCurrentID.setText("S"+(Integer.valueOf(rs3.getString(1).replaceAll("S",""))+1));
        }}catch(Exception ex){ JOptionPane.showMessageDialog(null, ex.getMessage(), "ERROR", JOptionPane.ERROR_MESSAGE);}
    }
